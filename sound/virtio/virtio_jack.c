...
int virtsnd_jack_parse_cfg(struct virtio_snd *snd)
{
	...
	ret = virtsnd_ctl_query_info(snd, VIRTIO_SND_R_JACK_INFO, 0,
...
int virtsnd_jack_build_devs(struct virtio_snd *snd)
{
	...
	list_for_each_entry(pcm, &snd->pcm_list, list) {
		if (pcm->njacks) {
			pcm->jacks = devm_kcalloc(&vdev->dev, pcm->njacks,
						  sizeof(*pcm->jacks),
						  GFP_KERNEL);
			if (!pcm->jacks)
				return -ENOMEM;
	...
	for (i = 0; i < snd->njacks; ++i) {
		...
		ret = snd_jack_new(snd->card, virtsnd_jack_get_label(jack),
		...
		pcm = virtsnd_pcm_find(snd, jack->nid);
		if (IS_ERR(pcm))
			return PTR_ERR(pcm);

		pcm->jacks[pcm->njacks++] = jack;
	}
