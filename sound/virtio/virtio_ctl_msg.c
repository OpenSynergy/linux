...
static int msg_timeout_ms = MSEC_PER_SEC;
...
int virtsnd_ctl_msg_send(struct virtio_snd *snd, struct virtio_snd_msg *msg)
{
	...
	ret = virtqueue_add_sgs(queue->vqueue, psgs, out_nsgs, in_nsgs, msg,
				GFP_ATOMIC);
	if (!ret)
			      ...
			      struct virtio_snd_msg *msg)
{
	...
	ret = virtsnd_ctl_msg_send(snd, msg);
	if (ret)
		goto on_failure;

	if (msg_timeout_ms)
		js = msecs_to_jiffies(msg_timeout_ms);
	else
		js = msecs_to_jiffies(MSEC_PER_SEC);

	ret = wait_for_completion_interruptible_timeout(&msg->notify, js);
	if (ret <= 0) {
		if (!ret) {
