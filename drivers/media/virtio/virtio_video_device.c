...
int virtio_video_buf_init(struct vb2_buffer *vb)
{
	int ret = 0;
	unsigned int i, j;
	uint32_t resource_id, offset = 0, nents = 0;
...
void virtio_video_buf_cleanup(struct vb2_buffer *vb)
{
	...
	ret = virtio_video_cmd_resource_destroy(vv, stream->stream_id,
						virtio_vb);
	if (ret)
		return;
...
static int virtio_video_device_open(struct file *file)
{
	...
	ret = virtio_video_cmd_stream_create(vv, stream_id, name);
	if (ret) {
...
virtio_video_device_create(struct virtio_video *vv)
{
	...
	INIT_LIST_HEAD(&vvd->controls_list);
