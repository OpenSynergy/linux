...
{
	int ret = 0;
	int resp_size = vv->max_cap_len;

	*resp_buf = kzalloc(vv->max_cap_len, GFP_KERNEL);
	if (IS_ERR(*resp_buf)) {
...
static int virtio_video_probe(struct virtio_device *vdev)
{
	...
	virtio_cread(vdev, struct virtio_video_config, max_cap_length,
		     &vv->max_cap_len);
	if (!vv->max_cap_len) {
		v4l2_err(&vv->v4l2_dev, "max_cap_len is zero\n");
		ret = -EINVAL;
		goto err_config;
	}
