...
struct video_control_level {
	uint32_t level;
};

struct video_control_profile {
	uint32_t profile;
	uint32_t min_level;
	uint32_t max_level;
	uint32_t level_skip_mask;
	uint32_t num_levels;
	struct video_control_level *levels;
};

struct video_control_format {
	struct list_head controls_list_entry;
	uint32_t format;
	uint32_t min_profile;
	uint32_t max_profile;
	uint32_t profile_skip_mask;
	uint32_t num_profiles;
	struct video_control_profile *profiles;
};
...
struct virtio_video_device {
	...
	struct list_head controls_list;
};
...
int virtio_video_g_fmt_vid_out(struct file *file, void *fh,
			       struct v4l2_format *f);
int virtio_video_g_fmt_vid_cap(struct file *file, void *fh,
			       struct v4l2_format *f);
