...
void virtio_video_dequeue_cmd_func(struct work_struct *work)
{
	...
	list_for_each_entry_safe(entry, tmp, &reclaim_list, list) {
		...
		if (resp->type >= cpu_to_le32(VIRTIO_VIDEO_RESP_ERR_UNSPEC))
			v4l2_dbg(1, vv->debug, &vv->v4l2_dev,
				   ...
				   const char *name)
{
	...
	strncpy(req_p->tag, name, sizeof(req_p->tag) - 1);
...
static void virtio_video_cmd_destroy_cb(struct virtio_video *vv,
					struct virtio_video_vbuffer *vbuf)
{
...
int virtio_video_cmd_resource_destroy(struct virtio_video *vv,
				      uint32_t stream_id,
				       struct virtio_video_buffer *virtio_vb)
{
	...
	req_p = virtio_video_alloc_req_resp
		(vv, &virtio_video_cmd_destroy_cb,
		 &vbuf, sizeof(*req_p),
		 sizeof(struct virtio_video_cmd_hdr), NULL);
	if (IS_ERR(req_p))
		return PTR_ERR(req_p);

	req_p->hdr.type = cpu_to_le32(VIRTIO_VIDEO_CMD_RESOURCE_DESTROY);
	req_p->hdr.stream_id = cpu_to_le32(stream_id);
...
int virtio_video_query_control_level(struct virtio_video *vv, void *resp_buf,
				     size_t resp_size, uint32_t profile)
{
	...
	ctrl_l->profile = cpu_to_le32(profile);

	vbuf->priv = &vv->got_levels;
				 ...
				 uint32_t control, uint32_t value)
{
	...
	case VIRTIO_VIDEO_CONTROL_PROFILE:
		...
		if (value == VIRTIO_VIDEO_PROFILE_UNDEFINED)
			return -EINVAL;
		break;
	case VIRTIO_VIDEO_CONTROL_LEVEL:
		size = sizeof(struct virtio_video_control_val_level);
		if (value == VIRTIO_VIDEO_LEVEL_UNDEFINED)
			return -EINVAL;
		break;
	case VIRTIO_VIDEO_CONTROL_BITRATE:
