...
int virtio_video_enc_init_ctrls(struct virtio_video_stream *stream)
{
	...
	list_for_each_entry(c_fmt, &vvd->controls_list, controls_list_entry) {
		switch (c_fmt->format) {
		case V4L2_PIX_FMT_H264:
			v4l2_ctrl_new_std_menu(&stream->ctrl_handler,
					       ...
					       c_fmt->max_profile,
					       c_fmt->profile_skip_mask,
					       c_fmt->min_profile);
			for (i = 0; i < c_fmt->num_profiles; i++) {
				c_profile = &c_fmt->profiles[i];
				if (c_profile->profile != c_fmt->min_profile)
					continue;
...
static int virtio_video_enc_s_fmt(struct virtio_video_stream *stream,
...
static int virtio_video_enc_s_fmt_vid_cap(struct file *file, void *fh,
					  struct v4l2_format *f)
{
	return virtio_video_enc_s_fmt(file2stream(file), f);
}

static int virtio_video_enc_s_fmt_vid_out(struct file *file, void *fh,
					  struct v4l2_format *f)
{
	return virtio_video_enc_s_fmt(file2stream(file), f);
}
